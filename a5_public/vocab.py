...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sentence in sents:
            ...
            for word in sentence:
                word_list = list(word)
                temp.extend([self.char2id[char] for char in word_list])
            temp.append(self.char2id['}'])
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        sents_list = self.words2charindices(sents)
        sent_var = pad_sents_char(sents_list, self.char2id['<pad>'])
